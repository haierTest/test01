...
public class TBQuotationZHXMLDecoder {
	...
	public void parseHead(Element head, Map paramMap, Map BLProposalMap)throws Exception{
		...
		Element REQUESTIP = head.element("REQUESTIP");
		request_ip = REQUESTIP.getText();
	}
	...
	public void parseBody(Element body, Map paramMap, Map BLProposalMap)throws Exception{
		...
		String isNewFlag = body.elementText("NEWFLAG");
		...
		if(SysConfig.getProperty("App_OperateSite").indexOf(","+OperateSiteBC+",")>-1){
			...
			ImmeValidFlagCI = body.elementText("IMMEVALIDFLAGCI");
			ImmeValidFlagBI = body.elementText("IMMEVALIDFLAGBI");
			ImmeValidStartDateCI = body.elementText("IMMEVALIDSTARTDATECI");
			ImmeValidStartDateBI = body.elementText("IMMEVALIDSTARTDATEBI");
			ImmeValidEndDateCI = body.elementText("IMMEVALIDENDDATECI");
			ImmeValidEndDateBI = body.elementText("IMMEVALIDENDDATEBI");
		}
		...
		parseCarOwner(body.element("CAROWNER_LIST"), paramMap, BLProposalMap);
		...
		paramMap.put("RENEWALCIFLAG", body.elementText("RENEWALCIFLAG"));
		paramMap.put("RENEWALBIFLAG", body.elementText("RENEWALBIFLAG"));
	...
	public void parseCar(Element carList, Map paramMap, Map BLProposalMap)throws Exception{
		...
		while(iterator.hasNext()){
			...
			double EXHAUSTSCALE = Double.parseDouble(carData.elementText("POWERSCALE"))/1000;
			...
			prpTitemCarSchema.setOtherNature(carData.elementText("ESPECIALLYCARTYPE"));
			...
			prpTitemCarSchema.setNewDeviceFlag(carData.elementText("NONEFLUCTUATEFLAG"));
			...
			prpTitemCarSchema.setSeatCountLB(carData.elementText("SEATCOUNTLB"));
			prpTitemCarSchema.setTonCountLB(carData.elementText("TONCOUNTLB"));
			...
			if(blProposalCI!=null){
				...
				if(SysConfig.getProperty("App_OperateSite").indexOf(","+OperateSiteBC+",")>-1){
					...
					prpTitemCarSchema.setNewCarFlag(carData.elementText("NEWVEHICLE"));
					prpTitemCarSchema.setOutLandCarFlag(carData.elementText("ECDEMICVEHICLE"));
				}
	...
	public void parseCarOwner(Element carOwnerList, Map paramMap, Map BLProposalMap)throws Exception{
		...
		while(iterator.hasNext()){
			...
			if(blProposalCI!=null){
				blProposalCI.getBLPrpTitemCar().getArr(0).setCarOwner(carownerData.elementText("OWNER"));
				blProposalCI.getBLPrpTitemCar().getArr(0).setCarInsuredRelation(carownerData.elementText("CARINSUREDRALATION"));
				blProposalCI.getBLPrpTitemCar().getArr(0).setCarOwnerNature(carownerData.elementText("OWNERNATURE"));
				blProposalCI.getBLPrpTitemCar().getArr(0).setInsuredTypeCode(carownerData.elementText("OWNERIDENTIFYTYPE"));
				blProposalCI.getBLPrpTitemCar().getArr(0).setOwnerAddress(carownerData.elementText("OWNERIDENTIFYNO"));
				
			}
			
			if(blProposalBI != null){

				blProposalBI.getBLPrpTitemCar().getArr(0).setCarOwner(carownerData.elementText("OWNER"));
				blProposalBI.getBLPrpTitemCar().getArr(0).setCarInsuredRelation(carownerData.elementText("CARINSUREDRALATION"));
				blProposalBI.getBLPrpTitemCar().getArr(0).setCarOwnerNature(carownerData.elementText("OWNERNATURE"));
				blProposalBI.getBLPrpTitemCar().getArr(0).setInsuredTypeCode(carownerData.elementText("OWNERIDENTIFYTYPE"));
				blProposalBI.getBLPrpTitemCar().getArr(0).setOwnerAddress(carownerData.elementText("OWNERIDENTIFYNO"));
	...
	public void parseDriver(Element driverList, Map paramMap, Map BLProposalMap)throws Exception{
		...
		while(iterator.hasNext()){		
			...
			prpTcarDriverSchema.setRiskCode(UtiPower.ChangeRiskCode0528("0508",comCode)); 
		...
		while(iterator.hasNext()){
			...
			if("1".equals(insuredData.elementText("INSUREDFLAG"))||"2".equals(insuredData.elementText("INSUREDFLAG"))){
			  if("1".equals(insuredData.elementText("INSUREDFLAG"))){
                ...
                if("0".equals(prpTinsuredSchema.getInsuredNature())){
	...
	public void parsePrptnewengage(Element prptnewengageList, Map paramMap, Map BLProposalMap)throws Exception{
		...
		while(iterator.hasNext()){
			...
			String strPROPOSALNO	= prptnewengageData.elementText("Proposal_No");
	    	String strFIRBENNEFITINFO	= prptnewengageData.elementText("Firbennefit_Info");
	    	String strMANAGERNAME	= prptnewengageData.elementText("Manager_Name");
	    	String strMANAGERMOBILENO	= prptnewengageData.elementText("ManagerMobileNo");
	    	String strSUNSHINECUSTOMER	= prptnewengageData.elementText("Sun_Shine_Customer");
	...
	public void parseKind(Element kindList, Map paramMap, BLProposal blProposal)throws Exception{
		...
		while(iterator.hasNext()){
			...
			double shortrate = Double.parseDouble(shortRate)*100;

...
public class CheckRuleInfo {
	...
	public void checkRule(BLProposal blProposal, Response ruleInfo, Map paramMap) throws Exception {
		...
		if(blProposal.getBLPrpTmain().getArr(0).getRiskCode().equals("0507")){
			...
			if(!CARPRICE.equals(bodyMap.get("PURCHASEPRICE"))){
				String Error_Code = "0200001";
				paramMap.put("Error_Code",Error_Code);
				
				paramMap.put("Error_Message","核心校验错误信息……"+"新车购置由"+"["+bodyMap.get("PURCHASEPRICE")+"]"+"变更为"+"["+CARPRICE+"]"+"");
				throw new Exception("核心校验错误信息……"+"新车购置由"+"["+bodyMap.get("PURCHASEPRICE")+"]"+"变更为"+"["+CARPRICE+"]"+"");

...
public class EndorseCarShipTaxValidEncoder {
    ...
    public void addVehicleTaxationQG(DbPool dbPool,BLEndorse blEndorse,StringBuffer buf) throws Exception{
        ...
        double isChgTax=0;
        if(blEndorse.getBLCIEndorValid().getSize()>0){
            String TaxAmendPremium = "";
            if(blEndorse.getBLCIEndorValid().getArr(0).getTaxAmendPremium()==null||"".equals(blEndorse.getBLCIEndorValid().getArr(0).getTaxAmendPremium()))
                TaxAmendPremium="0";
            else
                TaxAmendPremium=blEndorse.getBLCIEndorValid().getArr(0).getTaxAmendPremium();
            isChgTax=Double.parseDouble(TaxAmendPremium);
        }
        
        
        logger.info("isChgTax:::::::::::::::::::::::"+isChgTax);
        logger.info("blEndorse.getBLCIEndorValid().getArr(0).getTaxAmendDeclare():"+blEndorse.getBLCIEndorValid().getArr(0).getTaxAmendDeclare());
        
        
        UtiPower utiPower = new UtiPower();
        if("1".equals(blEndorse.getBLCIEndorValid().getArr(0).getTaxAmendDeclare())
                &&isChgTax<=0 && !utiPower.checkCarShipTaxJZ(blEndorse.getBLPrpPmain().getArr(0).getComCode(), blEndorse.getBLPrpPmain().getArr(0).getOperateDate())){
        
        	BLPrpPcarshipTax  bLPrpPcarshipTax = new  BLPrpPcarshipTax();
            bLPrpPcarshipTax.query(dbPool," endorseno='"+blEndorse.getBLPrpPhead().getArr(0).getEndorseNo()+"'");
            String TaxRelifFlag = bLPrpPcarshipTax.getArr(0).getTaxRelifFlag();
            ...
            if(utiPower.isCarShipTaxNB(blEndorse.getBLPrpPmain().getArr(0).getComCode())){
            ...
            addNode(buf, "TaxPayerName", bLPrpPcarshipTax.getArr(0).getTaxpayerName());
            addNode(buf, "TaxPayerIdentificationCode", bLPrpPcarshipTax.getArr(0).getTaxpayerIdentifier());
            if("1".equals(TaxRelifFlag) ){
                ...
                addNode(buf, "DeclareDate", correctDate(blEndorse.getBLPrpPmain().getArr(0).getOperateDate()));
                buf.append("<Derate>");
                addNode(buf, "DeductionDueCode", relifReasonEncoder(bLPrpPcarshipTax.getArr(0).getRelifReason()));
                addNode(buf, "DeductionDueType", baseTaxationEncoder(bLPrpPcarshipTax.getArr(0).getBaseTaxation()));
                addNode(buf, "DeductionDueProportion", ""+(Double.parseDouble(ChgData.chgStrZero(bLPrpPcarshipTax.getArr(0).getFreeRate()))/100));
                addNode(buf, "Deduction", bLPrpPcarshipTax.getArr(0).getTaxRelief());
                addNode(buf, "DeductionDocumentNumber", bLPrpPcarshipTax.getArr(0).getPaidFreeCertificate());
                addNode(buf, "TaxDepartmentCode", bLPrpPcarshipTax.getArr(0).getTaxComCode());
                addNode(buf, "TaxDepartment", bLPrpPcarshipTax.getArr(0).getTaxComName());
                buf.append("</Derate>");
                buf.append("</CurrentTaxDue>");
                ...
                buf.append("<CurrentTaxDue>");
                addNode(buf, "DeclareDate", correctDate(blEndorse.getBLPrpPmain().getArr(0).getOperateDate()));
                addNode(buf, "TaxStartDate", correctDate(bLPrpPcarshipTax.getArr(0).getPayStartDate()));
                addNode(buf, "TaxEndDate", correctDate(bLPrpPcarshipTax.getArr(0).getPayEndDate()));
                addNode(buf, "TaxDepartment", bLPrpPcarshipTax.getArr(0).getTaxComName());
                addNode(buf, "TaxDocumentNumber", bLPrpPcarshipTax.getArr(0).getPaidFreeCertificate());
                buf.append("</CurrentTaxDue>");
            }
            addNode(buf, "AnnualTaxDue", bLPrpPcarshipTax.getArr(0).getTaxActual());
            addNode(buf, "SumTaxDefault", bLPrpPcarshipTax.getArr(0).getPreviousPay());
            addNode(buf, "SumOverdue", bLPrpPcarshipTax.getArr(0).getLateFee());
            addNode(buf, "SumTax", bLPrpPcarshipTax.getArr(0).getSumPayTax());

 ...
 *
 */
public class EndorseValidEncoder {
	
	private BLPrpCPitemCar blPrpCPitemCar = new BLPrpCPitemCar();
	
	
	private static Logger logger = Logger.getLogger(EndorseValidEncoder.class);
	...
	public String encode(DbPool dbPool, BLEndorse blEndorse) throws Exception {
		...
		logger.info("[批改确认发送报文]:"+buf.toString());
	...
	protected void addBody(DbPool dbPool,StringBuffer buf, BLEndorse blEndorse)
        ...
        if(blEndorse.getBLPrpPhead().getArr(0).getComCode().substring(0,2).equals("07")){
        ...
        if(UtiPower.isIdentifyCheck(strComCode, strRiskCode)){
	...
	protected void addBasePart(DbPool dbPool,StringBuffer buf, BLEndorse blEndorse)
		...
		String strCurrentTax = "0";
		String strFormerTax  = "0";
		String strLateFee    = "0";
		...
		if(strComCode.substring(0,2).equals("07")){
			strCurrentTax = blEndorse.getBLCIEndorValid().getArr(0).getCurrentTax();
			strFormerTax  = blEndorse.getBLCIEndorValid().getArr(0).getFormerTax();
			strLateFee    = blEndorse.getBLCIEndorValid().getArr(0).getLateFee();
			strChgPremium = String.valueOf(Double.parseDouble(ChgData.chgStrZero(strChgPremium)) +
			                Double.parseDouble(ChgData.chgStrZero(strCurrentTax)) +
			                Double.parseDouble(ChgData.chgStrZero(strFormerTax))  +
			                Double.parseDouble(ChgData.chgStrZero(strLateFee)));
		}
		...
		addNode(buf,"AMEND_POLICY_NO",strEndorseNo);
    ...
    protected void addAmendList(StringBuffer buf, BLEndorse blEndorse)
    	...
    	for(int i = 0;i<iKindCount;i++){
    		...
    		String flag = blEndorse.getBLPrpPitemKind().getArr(i).getFlag();
    		double douChgPremium= Double.parseDouble(ChgData.chgStrZero(strChgPremium));
    		if("0507".equals(strRiskCode)&&douChgPremium==0&&flag.length()>1&&"U".equals(flag.substring(0, 1)))
    			break;
    ...
    public void addCheckItemCar(StringBuffer buf, BLEndorse blEndorse) throws Exception {
    	String strUseType = "";
    ...
    protected void addVehicle(StringBuffer buf, BLEndorse blEndorse) throws Exception {
    	...
    	if(new UtiPower().savePlatformAdjustSwitchJS(blEndorse.getBLPrpPmain().getArr(0).getComCode())){
        	addNode(buf, "VEHICLE_MODEL", blPrpCPitemCar.getArr(0).getBrandName());
        }
    ...
    protected void addCheckVehicle(StringBuffer buf, BLEndorse blEndorse) throws Exception {
    	...
    	if(new UtiPower().savePlatformAdjustSwitchJS(blEndorse.getBLPrpPmain().getArr(0).getComCode())){
        	...
        	strEcdemicVehicleFlag = TransCodeCI.getEcdemicVehicleFlag(blEndorse.getBLPrpPmain().getArr(0).getComCode(), strLicenseNo);
        	if("Y".equals(blPrpCPitemCar.getArr(0).getHKFlag())){
        		strEcdemicVehicleFlag = "2";
        	}
        	addNode(buf, "ECDEMIC_VEHICLE_FLAG", strEcdemicVehicleFlag);
        	addNode(buf, "TRANSFER_DATE", correctDate(blPrpCPitemCar.getArr(0).getChgOwnerDate()));
        }
